...
**Живой бот: [@myasoomsbot](https://t.me/myasoomsbot)** — команда `/start` открывает магазин.
Магазин развёрнут на сервере и работает круглосуточно, независимо от домашнего компьютера.
...
> **Нужно развернуть на своём сервере?**
> Пошаговая инструкция по переносу на VPS с доменом, HTTPS и бэкапами —
> в [docs/DEPLOY.md](docs/DEPLOY.md). Готовые скрипты:
> [`scripts/vps-bootstrap.sh`](scripts/vps-bootstrap.sh) — в Docker, на чистой машине;
> [`scripts/vps-native.sh`](scripts/vps-native.sh) — службами systemd, если на сервере
> уже есть nginx и PostgreSQL или мало памяти.
...
* Оформление заказа: имя, телефон, доставка или самовывоз, адрес с автодополнением, дата,
...
* **Расписание доставки не проверяется** — дата принимается как текст,
